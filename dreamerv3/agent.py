...
class Agent(nj.Module):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    self.ultra_sonic_sensor = nets.MLP((3,), **config.rewhead, name='ultra_sonic_sensor')
  ...
  def policy(self, obs, carry, mode='train'):
    ...
    embed_separate = self.enc_separate(obs, bdims=1)
    prevact = jaxutils.onehot_dict(prevact, self.act_space)
    lat, out = self.dyn.observe(
        prevlat, prevact, embed, obs['is_first'], bdims=1)

    total_newlat_separate = [lat]
    total_out_separate = [out]

    for iter, embed_y in enumerate(embed_separate):
      newlat_separate, outs_separate = self.dyn.observe_with_separate(prevlat, prevact, embed_y, obs['is_first'], iter, bdims=1)
      total_newlat_separate.append(newlat_separate)
      total_out_separate.append(outs_separate)

    lat, out = self.dyn.compute_bayesian_surprise(total_out_separate, total_newlat_separate, out)
    #out = total_out_separate[-1]
    #lat = total_newlat_separate[-1]
    outs = {}
    if self.config.replay_context:
  ...
  def loss_sensor(self, data, carry, update=True):
    metrics = {}
    prevlat, prevact = carry
    # Replay rollout
    prevacts = {
    ...
    if self.config.imag_repeat > 1:
      ...
      investigate_startrew = treemap(
          lambda x: x.repeat(N, 0), (investigate_startrew))
    avoid_startout = {key: jnp.copy(value) for key, value in startout.items()}
    investigate_startout = {key: jnp.copy(value) for key, value in startout.items()}
    startact = cast(sample(self.actor(startout, bdims=1)))
    sonic_start = sg(self.ultra_sonic_sensor(startout,bdims=1).mean())
    investigate_startout['ultra_sonic_sensor'] = jnp.copy(sonic_start)
    avoid_startout['ultra_sonic_sensor'] = jnp.copy(sonic_start)
    ...
    avoid_startact = cast(sample(self.avoid_actor(avoid_startout,bdims=1)))
    investigate_startact = cast(sample(self.investigate_actor(investigate_startout,bdims=1)))
    ...
    avoid_outs, avoid_acts = treemap(
        lambda first, seq: jnp.concatenate([first, seq], 1),
        treemap(lambda x: x[:, None], (avoid_startout, avoid_startact)), (avoid_outs, avoid_acts))
    
    investigate_outs, investigate_acts = treemap(lambda x: x.swapaxes(0, 1), (investigate_outs, investigate_acts))
    investigate_outs, investigate_acts = treemap(
        lambda first, seq: jnp.concatenate([first, seq], 1),
        treemap(lambda x: x[:, None], (investigate_startout, investigate_startact)), (investigate_outs, investigate_acts))
  ...
  def loss_v0(self, data, carry, update=True):
    metrics = {}
    prevlat, prevact = carry
