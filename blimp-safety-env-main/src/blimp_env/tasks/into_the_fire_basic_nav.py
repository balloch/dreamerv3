...
def action_int_to_vec(action: int) -> np.ndarray:
    if action == 0:
        ...
        return np.array([1, 0, 0, 0]) # Forward
    elif action == 2:
        return np.array([-1, 0, 0, 0]) # Backward
    elif action == 3:
        return np.array([0, 1, 0, 0]) # Right
    elif action == 4:
        return np.array([0, -1, 0, 0]) # Left
    elif action == 5:
        return np.array([0, 0, +1, 0]) # Up
    elif action == 6:
        return np.array([0, 0, -1, 0]) # Down
    # Do not permit yawing.
...
p0.renderer.setColor(LVector4(0.00, 1.00, 0.00, 1.00))
...
"""
    ...
    def _setup_danger_visualizer(self):
        ...
        mat = Material()
        mat.set_diffuse(Vec4(0, 1, 0, .8)) #Geigh Zollicoffer
...
class GeneralObjectManager:
    def __init__(self, fire_mgr=None, balloon_mgr=None, debug: bool = False):
        self.debug = debug
        self._objs = {}
...
class Base(ShowBaseSingleton):
    ...
    def _setup_clock(self):
        ...
        self.clock.dt = 1.0 / 60 # Is this 60 Hz or 1 Hz? I assume 60 Hz...
    ...
    def _setup(self):
        ...
        self.object_mgr = GeneralObjectManager(debug=self.debug)
    ...
    def _reset_fire(self, locations: List[Tuple[float, float, float]], danger_radius: float) -> None:
        ...
        self.object_mgr._objs=dict(self.fire_mgr._fires)
    ...
    def _reset_balloons(self, locations: List[Tuple[float, float, float]], reward_zone_radius: float) -> None:
        ...
        self.object_mgr._objs.update(dict(self.balloon_mgr._balloons))
...
class IntoTheFireBasicNavEnv(gym.Env):
    ...
    def __init__(
        ...
        self.action_space = gym.spaces.Discrete(7) # NOTE: this was 6.
    ...
    # Simulate the physics forward given an array u, instead of a discrete action.
    def simulate_physics_u(self, u: np.ndarray):
        blimp_state = self.base.get_blimp_state()
        pose = blimp_state["pose"]
        velocity = blimp_state["velocity"]
        angular_velocity = blimp_state["angular_velocity"]
    ...
    def physics_prediction(self, action: int, n=1) -> np.ndarray:
        # Predict the next state generated by the physics model
        blimp_state = self.base.get_blimp_state()
        pose = blimp_state["pose"]
        velocity = blimp_state["velocity"]
        angular_velocity = blimp_state["angular_velocity"]
        X = np.concatenate([velocity, angular_velocity, pose]).copy()
        U = action_int_to_vec(action)
        U = U * np.array([0.01, 0.01, 0.025, 0.002])
        for _ in range(self.action_repeat):
            sim_out = self.blimp.step(X.copy(), U.copy(), self.base.dt)
            X = sim_out.copy()
        return X # The one-step prediction
        ...
        for t in range(self.action_repeat):
            if self.dynamics == "simple":
                ...
                if type(action) == np.ndarray:
                    self.simulate_physics_u(action)
                    break # Do not re-apply
                else:
    ...
    def _get_reward_and_episode_status(self) -> Dict[str, Any]:
        ...
        self.base.object_mgr._objs = dict(self.base.balloon_mgr._balloons)
        self.base.object_mgr._objs.update(dict(self.base.fire_mgr._fires))
        return info
